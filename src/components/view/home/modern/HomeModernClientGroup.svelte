...
<div>
	...
	<div class="flex flex-wrap gap-1">
		{#each clients.slice(0, maxVisible) as client (client.id)}
			<a
		...
		{/each}
		{#if clients.length > maxVisible}
			<span class="px-2 py-1 text-xs text-gray-400">+{clients.length - maxVisible}</span>
		{/if}
	</div>
</div>
