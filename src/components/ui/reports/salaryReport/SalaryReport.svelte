<script lang="ts">
	...
	const detailBuckets = (trainer: SalaryReportTrainer | null) =>
		trainer
			? [
					{
						key: 'weekday',
			...
			<div class="mb-4 flex items-start justify-between gap-4">
				<div>
					...
					<p class="text-text/70 text-sm">
						{selectedTrainer.email ?? 'Ingen e-post'} · {selectedTrainer.locationName ??
							'Ingen plats'}
					</p>
				</div>
				...
				{#each detailBuckets(selectedTrainer) as bucket}
					<details class="rounded-sm border border-gray-200 p-3" open>
						<summary class="cursor-pointer font-semibold">
							{bucket.label} ({bucket.entries.length} st, {minutesToHours(bucket.minutes).toFixed(
								2
							)} h)
						</summary>
