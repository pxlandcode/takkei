<script lang="ts">
	...
	import SalaryReportDetailsPopup from './SalaryReportDetailsPopup.svelte';
	...
	import type { SalaryReportResponse, SalaryReportTrainer } from './salaryReportTypes';
	...
	const headers = [
		...
		{ label: 'Frånvaro', key: 'absence', width: '260px' },
	...
	function mapTrainerToRow(trainer: SalaryReportTrainer) {
		return {
			...
			absence: formatAbsenceSummary(trainer),
			actions: [
				{
					type: 'button',
					label: 'Visa detaljer',
					icon: 'Eye',
					variant: 'secondary',
					action: () => openTrainerDetails(trainer)
				}
			]
		};
	}
	...
	function openTrainerDetails(trainer: SalaryReportTrainer) {
		openPopup({
			header: trainer.name || 'Lönedetaljer',
			icon: 'Eye',
			component: SalaryReportDetailsPopup,
			props: { trainer },
			maxWidth: '960px'
		});
	}
