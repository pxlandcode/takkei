...
<div class="flex flex-col gap-6">
        ...
        <div class="grid gap-4 md:grid-cols-2">
                <div>
                        <h4 class="text-sm font-semibold uppercase text-gray-500">Mottagare ({mail.recipients_count})</h4>
                ...
                <div class="space-y-2 text-sm">
                        {#if mail.header}
                                <div>
                                        <span class="font-semibold">Rubrik:</span> {mail.header}
                                </div>
                        {/if}
                        {#if mail.subheader}
                                <div>
                                        <span class="font-semibold">Underrubrik:</span> {mail.subheader}
                                </div>
                        {/if}
                        <div>
                                <span class="font-semibold">Status:</span> {mail.status ?? 'sent'}
                        </div>
                        {#if mail.error}
                                <div class="text-red-600">
                                        <span class="font-semibold">Fel:</span> {mail.error}
                                </div>
                        {/if}
                </div>
        </div>
        ...
        <div class="rounded border border-gray-200 bg-white p-4">
                <h4 class="text-sm font-semibold uppercase text-gray-500">Innehåll</h4>
        ...
        <div class="flex justify-end">
                <Button text="Öppna i Mailutskick" iconLeft="Mail" variant="primary" on:click={openInMailComponent} />
        </div>
</div>
