...
export const GET: RequestHandler = async ({ url }) => {
	...
	try {
		const { buffer, filename } = await buildBookingReportWorkbook(filters);
		...
		console.error('Failed to export bookings report', error);
		return new Response('Failed to export bookings report', { status: 500 });
	}
};
