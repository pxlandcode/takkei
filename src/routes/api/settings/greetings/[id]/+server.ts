...
export const PATCH: RequestHandler = async ({ locals, params, request }) => {
	...
	try {
		...
		console.error('Invalid JSON payload for greeting update', error);
	...
	try {
		...
		return json({ data: mapGreetingRow(updated) });
	} catch (error) {
...
export const DELETE: RequestHandler = async ({ locals, params }) => {
	...
	try {
		const rows = await query('DELETE FROM greetings WHERE id = $1 RETURNING id', [id]);
