...
function normalizeNote(raw: BookingNote): BookingNote {
	const note_kind =
		raw.note_kind ??
...
export async function fetchBookingNotes(
	bookingId: number,
	...
	try {
		...
		const notes: BookingNote[] = await response.json();
...
export async function linkNoteToBooking(
	...
	try {
		...
		return normalizeNote(result as BookingNote);
	} catch (error) {
...
export async function unlinkNoteFromBooking(
	noteId: number,
	fetchFn: typeof fetch
): Promise<boolean> {
	try {
