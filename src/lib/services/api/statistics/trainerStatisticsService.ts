...
type TableCategory = 'weekday' | 'weekendHoliday' | 'demo' | 'flight' | 'education' | 'practice';
...
export async function getTrainerStatisticsService(
	...
	const latestEnd = allRanges.reduce(
		(max, range) => (!max || range.endExclusive > max ? range.endExclusive : max),
		''
	);
	...
	const detailedBookings = combineBookingsWithLateCancellations(
		processedActive,
		processedCancelled
	);
	const debiterbaraBookings = detailedBookings.filter(isBillableBooking);
	...
	const debiteradeBookings = debiterbaraBookings;

	const debiteradeWeekMinutes = sumMinutes(debiteradeBookings, ranges.currentWeek, nowStockholm);
	const debiteradeMonthMinutes = sumMinutes(debiteradeBookings, ranges.currentMonth, nowStockholm);
	...
	const previousWeekCancellations = summarizeCancellations(processedCancelled, ranges.previousWeek);
	...
	const table = buildTable(detailedBookings, debiterbaraBookings, processedCancelled, customRange);
	...
	return {
		...
		demotraningar: {
			...
			deltaLabel: formatDeltaLabel(monthTryOutCount - previousMonthTryOutCount, 'prevMonth')
		},
		debiterbaraTimmar: {
...
async function loadTrainerBookings(
	...
	try {
		const rows = await query<BookingRow>(buildTrainerBookingsQuery(true, statusPredicate), params);
...
function combineBookingsWithLateCancellations(
	activeBookings: ProcessedBooking[],
	cancelledBookings: ProcessedBooking[]
) {
	return activeBookings.concat(cancelledBookings.filter((booking) => booking.isLateCancellation));
}

function isBillableBooking(booking: ProcessedBooking) {
	return booking.category !== 'practice';
}
...
function formatDeltaLabel(value: number, context: DeltaContext, unit?: string) {
	...
	const formattedValue = Number.isInteger(roundedAbs)
		? roundedAbs.toString()
		: roundedAbs.toFixed(1);
...
function buildTable(
	detailedBookings: ProcessedBooking[],
	billableBookings: ProcessedBooking[],
	cancelledBookings: ProcessedBooking[],
	range: RangeDescriptor
) {
	const detailedInRange = detailedBookings.filter((booking) =>
	...
	const rows = Object.entries(TABLE_CATEGORY_LABELS).map(([category, label]) => {
		...
		const categoryBookings = detailedInRange.filter((booking) => booking.category === categoryKey);
	...
	const totals = {
		...
		hours: minutesToHours(
			billableInRange.reduce((sum, booking) => sum + booking.durationMinutes, 0)
		),
		lateCancellations: cancelledInRange.filter(
			(booking) => booking.isLateCancellation && isBillableBooking(booking)
		).length,
		obTotal: minutesToHours(totalObMinutes)
	};
