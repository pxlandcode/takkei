...
export async function POST({ request }) {
	try {
		...
		const clientResult = await query(
			`
          INSERT INTO clients (customer_id, firstname, lastname, email, phone, person_number, key, active, created_at, updated_at)
            VALUES ($1, $2, $3, $4, $5, $6, $7, true, NOW(), NOW())
            RETURNING id
        `,
			[customerId, firstname, lastname, email, phone, personnummer, clientKey]
		);
